  ...
  foreach(externalName IN ITEMS $ENV{DCG_SOLUTION_EXTERNALS})
    if(NOT EXISTS "${CMAKE_SOURCE_DIR}/externals/${externalName}")
      if("$ENV{DCG_EXTERNAL_${externalName}_TYPE}" STREQUAL DCG_SUBMODULE)
        execute_process(COMMAND git submodule add "$ENV{DCG_EXTERNAL_${externalName}_LOCATION}" "${CMAKE_SOURCE_DIR}/externals/${externalName}")
      else()
...
function(DCG_create_cmake_file projectName isTestProject)
  ...
  foreach(fileId IN ITEMS $ENV{DCG_PROJECT_${projectName}_FILES})
    set(fileSourceSuffix ".c")
    if("$ENV{${fileId}_TYPE}" STREQUAL "DCG_CPP")
      set(fileSourceSuffix ".cpp")
    endif()
    ...
    if("$ENV{${fileId}_HAS_SOURCE}")
      list(APPEND projectSourceFiles "\${CMAKE_CURRENT_SOURCE_DIR}/source/$ENV{${fileId}_PATH}${fileSourceSuffix}")
    endif()

    if("$ENV{${fileId}_HAS_TEST}")
      list(APPEND projectTestFiles "\${CMAKE_CURRENT_SOURCE_DIR}/$ENV{${fileId}_PATH}${fileSourceSuffix}")
    endif()
  endforeach()
  ...
  if(${isTestProject})
    ...
    string(APPEND projectDeclarationSection "set_property(TARGET ${projectTarget} PROPERTY FOLDER tests/${projectName})\n\n")
  else()
    if("$ENV{DCG_PROJECT_${projectName}_TYPE}" STREQUAL DCG_LIB)
      ...
      string(APPEND projectDeclarationSection "set_property(TARGET ${projectTarget} PROPERTY FOLDER libraries/${projectName})\n\n")
    ...
    elseif("$ENV{DCG_PROJECT_${projectName}_TYPE}" STREQUAL DCG_INTERFACE)
      ...
      string(APPEND projectDeclarationSection "add_executable(${projectTarget})\n\n")
      string(APPEND projectDeclarationSection "set_property(TARGET ${projectTarget} PROPERTY FOLDER executables/${projectName})\n\n")
    endif()
  endif()
