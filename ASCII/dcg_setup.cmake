...
function(DCG_add_interface_source_group targetName)
  ...
  foreach(linkLibrary IN ITEMS ${targetLinkLibs})
    if(NOT "$ENV{DCG_PROJECT_${linkLibrary}_TYPE}" STREQUAL "")
...
function(DCG_PROJECT name type)
  ...
  foreach(value RANGE 2 "${lastValue}")
    ...
    if("${argument}" STREQUAL "PRIVATE_DEPENDS" OR "${argument}" STREQUAL "PUBLIC_DEPENDS" OR "${argument}" STREQUAL "TEST_DEPENDS")
      ...
      elseif("${argMode}" STREQUAL "TEST_DEPENDS")
        list(APPEND testDependencies "${argument}")
      endif()
    endif()
  endforeach()
  ...
  set("ENV{DCG_PROJECT_${name}_TEST_DEPENDENCIES}" "${testDependencies}")
...
function(DCG_FILE_PUBLIC_RESOURCE name)
  DCG_FILE("${name}" DCG_RESOURCE TRUE FALSE FALSE "")
endfunction()

function(DCG_FILE_PRIVATE_RESOURCE name)
  DCG_FILE("${name}" DCG_RESOURCE FALSE TRUE FALSE "")
endfunction()
...
function(DCG_EXTERNAL_SUBMODULE_CUSTOM_CMAKE name address)
  DCG_append_env_list("DCG_SOLUTION_EXTERNALS" "${name}")
  set("ENV{DCG_EXTERNAL_${name}_TYPE}" DCG_SUBMODULE_CUSTOM_CMAKE)
  set("ENV{DCG_EXTERNAL_${name}_LOCATION}" "${address}")
endfunction()
...
function(DCG_END)
  ...
  if("$ENV{DCG_DEBUG_PRINT_STRUCTURE}")
    ...
    foreach(projectName IN ITEMS $ENV{DCG_SOLUTION_PROJECTS})
      ...
      message("      Test Dependencies : $ENV{DCG_PROJECT_${projectName}_TEST_DEPENDENCIES}")
...
function(DCG_safe_move souceFile destinationFile)
  get_filename_component(sourceDir "${souceFile}" DIRECTORY)
  get_filename_component(sourceName "${souceFile}" NAME)
  get_filename_component(destinationDir "${destinationFile}" DIRECTORY)
  get_filename_component(destinationName "${destinationFile}" NAME)

  DCG_get_unique(tempNameUnique)
  set(tempName "DCG_temp_file_${tempNameUnique}")

  file(RENAME "${sourceDir}/${sourceName}" "${sourceDir}/${tempName}")
  file(COPY "${sourceDir}/${tempName}" DESTINATION "${destinationDir}")
  file(REMOVE "${sourceDir}/${tempName}")
  file(RENAME "${destinationDir}/${tempName}" "${destinationDir}/${destinationName}")
endfunction()
  ...
  foreach(projectName IN ITEMS $ENV{DCG_SOLUTION_PROJECTS})
    foreach(fileId IN ITEMS $ENV{DCG_PROJECT_${projectName}_FILES})
      foreach(sourceArea IN ITEMS ${sourceAreas})
        set(fileType ".c")
        set(fileSuffix "")
        set(shouldWriteContent TRUE)

        if("$ENV{${fileId}_TYPE}" STREQUAL "DCG_CPP")
          set(fileType ".cpp")
        elseif("$ENV{${fileId}_TYPE}" STREQUAL "DCG_RESOURCE")
          set(fileType "")
          set(shouldWriteContent FALSE)
        endif()
        ...
        if(sourceArea STREQUAL "include")
          ...
          if(NOT "$ENV{${fileId}_TYPE}" STREQUAL "DCG_RESOURCE")
            set(fileType ".h")
          endif()
  ...
  foreach(externalName IN ITEMS $ENV{DCG_SOLUTION_EXTERNALS})
    ...
    if(NOT EXISTS "${CMAKE_SOURCE_DIR}/externals/${externalName}")
      ...
      if("$ENV{DCG_EXTERNAL_${externalName}_TYPE}" STREQUAL DCG_SUBMODULE)
        ...
        execute_process(COMMAND git submodule add "$ENV{DCG_EXTERNAL_${externalName}_LOCATION}" "./externals/${externalName}" WORKING_DIRECTORY "${CMAKE_SOURCE_DIR}")
        set(hasSubmodules TRUE)
      elseif("$ENV{DCG_EXTERNAL_${externalName}_TYPE}" STREQUAL DCG_SUBMODULE_CUSTOM_CMAKE)
        if (NOT EXISTS "${CMAKE_SOURCE_DIR}/externals/${externalName}")
          file(MAKE_DIRECTORY "${CMAKE_SOURCE_DIR}/externals/${externalName}")
        endif()
        DCG_message_if("$ENV{DCG_DEBUG_PRINT_SUBMODULE_EXTERNALS}" "    It's a submodule with custom cmake, running 'git submodule add \"$ENV{DCG_EXTERNAL_${externalName}_LOCATION}\" \"./externals/${externalName}/${externalName}\"' and creating '${CMAKE_SOURCE_DIR}/externals/${externalName}/CMakeLists.txt'")
        execute_process(COMMAND git submodule add "$ENV{DCG_EXTERNAL_${externalName}_LOCATION}" "./externals/${externalName}/${externalName}" WORKING_DIRECTORY "${CMAKE_SOURCE_DIR}")
      ...
      if("$ENV{DCG_EXTERNAL_${externalName}_TYPE}" STREQUAL DCG_SUBMODULE)
        ...
        set(hasSubmodules TRUE)
      elseif("$ENV{DCG_EXTERNAL_${externalName}_TYPE}" STREQUAL DCG_SUBMODULE_CUSTOM_CMAKE)
        DCG_message_if("$ENV{DCG_DEBUG_PRINT_SUBMODULE_EXTERNALS}" "    It's a submodule with custom cmake")
        set(hasSubmodules TRUE)
      else()
...
function(DCG_create_cmake_file projectName isTestProject)
  ...
  if(${isTestProject})
    ...
    string(APPEND projectDependenciesSection "  gmock\n")
    string(APPEND projectDependenciesSection "  gtest_main\n")
    if(NOT "$ENV{DCG_PROJECT_${projectName}_TEST_DEPENDENCIES}" STREQUAL "")
      foreach(dependency IN ITEMS $ENV{DCG_PROJECT_${projectName}_TEST_DEPENDENCIES})
  ...
  if(${isTestProject})
    ...
    foreach(dependency IN ITEMS $ENV{DCG_PROJECT_${projectName}_TEST_DEPENDENCIES})
      if(NOT "$ENV{DCG_PROJECT_${dependency}_TYPE}" STREQUAL "")
